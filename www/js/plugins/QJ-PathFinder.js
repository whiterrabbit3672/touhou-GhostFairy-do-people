 ...
 *
 * 2.虽然算法和MV原本的寻路一样都是A*算法，但是通过预处理和数据再利用，其运算效率比MV
 ...
 *   来判断寻路是否成功(返回一个布尔值)，只有当寻路完全通通畅时才算寻路成功。
 *   
 *   在执行这条语句之后，还可以在脚本或者条件分歧中使用
 *   this.character(characterId).pathFinderMoveSucceed()
 *   来判断寻路后是否移动成功，因为寻路即使不成功，角色也可能可以向目标移动，故而我建议主要使用这个来判定。
 *
...
const playerTouchMaxLength = eval(parameters["playerTouchMaxLength"]);
const playerTouchDiaMove = eval(parameters["playerTouchDiaMove"]);

const playerDiaMove = eval(parameters["playerDiaMove"]);
const playerMaxLength = eval(parameters["playerMaxLength"]);
const palyerCool = Number(parameters["palyerCool"] || 60);

const eventDiaMove = eval(parameters["eventDiaMove"]);
const eventMaxLength = eval(parameters["eventMaxLength"]);
const eventCool = Number(parameters["eventCool"] || 60);
//=============================================================================
//
//=============================================================================
...
Game_Map.prototype.createNewMapBlankGridPF = function() {
    let mapData = new Array();
    for (let j=0,jl=this.height();j<jl;j++) {
        mapData[j] = new Array();
        for (let i=0,il=this.width();i<il;i++) {
    ...
    return new PathFinderGrid(mapData);
};
//=============================================================================
//Game_Character
//=============================================================================
Game_Character.prototype.pointToMovePF = function(orginXy,tarXy) {
...
Game_Character.prototype.pathFinderXy = function(startX,startY,tarX,tarY,extraData) {
    //================================================
    let range = extraData.range>=0 ? extraData.range : 1;
    //================================================
    let showTrace = !!extraData.showTrace;
    //================================================
    let maxLength = extraData.maxLength>=1 ? extraData.maxLength : (this===$gamePlayer?playerMaxLength:eventMaxLength);
    //================================================
    let move = !!extraData.move;
    //================================================
    let dia = extraData.dia===undefined ? (this===$gamePlayer?playerDiaMove:eventDiaMove) : extraData.dia;
    //================================================
    this.setPathFinderSucceed(false);
    //================================================
    if ((move&&this._pathFinderCoolCount>0)||this.isMoving()) {
        this.setPathFinderMoveSucceed(true);
        return 0;
    } else {
        if (move) {
            this.setPathFinderMoveSucceed(false);
        }
    }
    //================================================
    let mapPointer = $gameMap;
    tarX = mapPointer.canvasToGridX(tarX);
    tarY = mapPointer.canvasToGridY(tarY);
    startX = mapPointer.canvasToGridX(startX);
    startY = mapPointer.canvasToGridY(startY);
    if (!mapPointer.isValid(startX,startY) || !mapPointer.isValid(tarX,tarY)) return 0;
    //================================================
    ...
    if (this.isThrough() || this.isDebugThrough()) {
        gridData = mapPointer.createNewMapBlankGridPF();
    } else {
        gridData = QJ.PFEX.mapOrgin.clone();
        for (let i of mapPointer._events) {
            if (!i) continue;
            if (i.isNormalPriority()) {
                x = mapPointer.canvasToGridX(i.x);
                y = mapPointer.canvasToGridY(i.y);
                if (mapPointer.isValid(x,y)) {
                    gridData.setWalkableAt(x,y,false);
                }
            }
        }
        //事件专属
        if (this!==$gamePlayer && this.isNormalPriority()) {
            let playerPointer = $gamePlayer;
            x = mapPointer.canvasToGridX(playerPointer.x);
            y = mapPointer.canvasToGridY(playerPointer.y);
            if (mapPointer.isValid(x,y) && playerPointer.isCollided(x,y)) {
                gridData.setWalkableAt(x,y,false);
                //玩家的确可被碰撞时，队友才可能被碰撞。
                playerPointer.followers().visibleFollowers().forEach((p)=>{
                    x = mapPointer.canvasToGridX(p.x);
                    y = mapPointer.canvasToGridY(p.y);
                    if (mapPointer.isValid(x,y)) {
                        gridData.setWalkableAt(x,y,false);
                    }
                });
        ...
        if (!boat.isThrough()) {
            x = mapPointer.canvasToGridX(boat.x);
            y = mapPointer.canvasToGridY(boat.y);
            if (mapPointer.isValid(x,y)) {
                gridData.setWalkableAt(x,y,false);
            }
        }
        ...
        if (!ship.isThrough()) {
            x = mapPointer.canvasToGridX(ship.x);
            y = mapPointer.canvasToGridY(ship.y);
            if (mapPointer.isValid(x,y)) {
                gridData.setWalkableAt(x,y,false);
            }
        }
    ...
    this.setPathFinderSucceed(pathFinder.canArrive);
    ...
    if (path.length<=1) {
        ...
        this.setPathFinderMoveSucceed(this.isMovementSucceeded());
        if (!this.isMovementSucceeded()) {
            this.turnTowardPointPF(tarX,tarY);
        }
        this._pathFinderCoolCount = this===$gamePlayer?palyerCool:eventCool;
        return 0;
    } else {
        if (move) {
            if (dia && Math.abs(range - Math.sqrt(distance))<1) {
                this.moveTowardXyPF(tarX,tarY);
            } else {
                this.pointToMovePF([startX,startY],path[1])
            };
            this.setPathFinderMoveSucceed(this.isMovementSucceeded());
            return 1;
        } else {
...
Game_Character.prototype.pathFinderChar = function(characterId,range,showTrace = false,move = true,maxLength = 0) {
...
Game_Character.prototype.fpXy = function(tarX,tarY,range,showTrace = false,move = true,maxLength = 0) {
    return this.pathFinderXy(this.x,this.y,tarX,tarY,{
        range:range,
        showTrace:showTrace,
        maxLength:maxLength,
        move:move
...
Game_Character.prototype.fpChar = function(character,range,showTrace = false,move = true,maxLength = 0) {
    return this.pathFinderChar(character,range,showTrace,move,maxLength);
};
//=============================================================================
//
//=============================================================================
...
Game_Character.prototype.pathFinderMoveSucceed = function() {
    return !!this._pathFinderMoveSucceed;
};
Game_Character.prototype.setPathFinderSucceed = function(value) {
    this._pathFinderSucceed = value;
};
Game_Character.prototype.setPathFinderMoveSucceed = function(value) {
    this._pathFinderMoveSucceed = value;
};
//=============================================================================
//
//=============================================================================
        ...
        if (data) {
            if (data.remX!=goalX||data.remY!=goalY) {
                dir = this.pathFinderXy(this.x,this.y,goalX,goalY,{
                    ...
                    move:false,
                    range:0,
                    dia:playerTouchDiaMove,
                    maxLength:playerTouchMaxLength
                });
                if (dir>0 && this._pathFinderRealTarget) {
